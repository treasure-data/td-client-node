...
export class TDClient {
   ...
   /**
     *  Submit Trino type job
     *  @param {string} db - The name of database
     *  @param {string} query - The Trino query which run on given database
     *  @param {object} opts - Supported options are `result`,
     *                         `priority` and `retry_limit`
     *  @param {function} callback - Callback function which receives
     *                               error object and results object
     *
     *  @see {@link https://docs.treasuredata.com/categories/trino}
     */
    trinoQuery(db, query, opts, callback) {
        if (typeof opts === 'function') {
            callback = opts;
            opts = {};
        }
        this._query(db, 'trino', query, opts, callback)
    }
